...
RUN cmake -B /build -S /src \
    ...
    && set +e \
    && ZSTD_CHECK_OUTPUT=$(/build/perf_analyzer/src/perf-analyzer-build/perf_analyzer -m zstd_check -u localhost:1 -i grpc --grpc-compression-algorithm zstd --concurrency-range 1 --measurement-interval 1 2>&1) \
    && set -e \
    && echo "$ZSTD_CHECK_OUTPUT" \
    && ! echo "$ZSTD_CHECK_OUTPUT" | grep -q "Failed to parse --grpc-compression-algorithm"
